...
#define CORNUS_THICKET_VERSION "2.2.06"
...
struct ThicketRun {  // templated struct to work around absence of inline variables in C++14
    Option opt_version;
    Option opt_clean_only;
    Option opt_quiet;
    Option opt_force; // do not ask anything
    Option opt_method;
    Option opt_clean_method;  // erase method (imprint or mounts)
    Option opt_print_tree;
    Option opt_dry_run;
    Option opt_root_lev;
    Option opt_variable; // multivalue
    Option opt_end_of_options;

    std::array<Option*, 11> all_options;

    ThicketRun() :
     opt_version("-version"),
     opt_clean_only("-c"),
     opt_quiet("-q"),
     opt_force("-f"), // do not ask anything
     opt_method("-m", "symlinks"),
     opt_clean_method("-em", "imprint"),  // erase method (imprint or mounts)
     opt_print_tree("-p"),
     opt_dry_run("-d"),
     opt_root_lev("-root_lev", true),
     opt_variable("-var", true, true), // multivalue
     opt_end_of_options("--"),
    ...
    int run_thicket(Context& ctx){
        ...
        int& error_count = getErrorCounts().error_count;
        int& shown_errors_count = getErrorCounts().shown_errors_count;
    ...
    bool parse_option(char* s, bool& err){
